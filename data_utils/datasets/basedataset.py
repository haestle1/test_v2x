...
# lidar point cloud pre processing을 위한 기능
# utils의 dataset을 기반으로 point cloud를 로딩하고, 증강하며 노이즈를 추가
# 다양한 lidar point cloud data 처리에서 base class로 사용
# 구체적인 데이터 처리 로직은 자식 클래스에서 구현
class BaseDataset(Dataset):
    ...
    """
    # pre processing parameters를 받아서 초기화
    # data augmentation, time delay, localization error 등을 설정
    ...
    # 주어진 idx(인덱스)에 해당하는 데이터를 반환.
    # senario data base에서 해당하는 시나리오와 time stamp를 찾아 데이터를 로드
    ...
    # point cloud를 BEV map으로 변환
    # 이는 point cloud를 BEV map으로 변환하여 pre/post processing을 진행하는 모델에서 필요
    # V2X-ViT는 voxel 기반으로 data 처리를 하기에 필요하지 않다.
    ...
    # data augmentation을 수행.
    # data_augmentor 객체의 method을 호출
    ...
    # 훈련을 위한 batch data를 정리. batch data를 적절하게 변환
